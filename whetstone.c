...
int main()
{
  ...
  printf("Calibrate (Estimate number of loops for final test using around 30 seconds)\n");
  do
   {
    TimeUsed=0;
   ...
   if (TimeUsed > 0) xtra = (long)((SPDP)(duration * xtra) / TimeUsed);//total expected bench time is 'duration' 
   if (xtra < 1) xtra = 1;
  ...
  return 0;           
    ...
    void whetstones(long xtra, long x100, int calibrate)
      {
        ...
        pout("N2 flt pnt fcn call\0",(float)(n2*96)*(float)(xtra),
                             1,e1[3],timeb,calibrate,2);
        ...
        pout("N4 integer arith   \0",(float)(n4*15)*(float)(xtra),
                                 2,x,timeb,calibrate,4);
        ...
        pout("N5 mul,sin,cos,atan\0",(float)(n5*26)*(float)(xtra),
                                 1,y,timeb,calibrate,5);
        ...
        timeb = dtime()-timea;
        pout("N6 flt pnt fcn call\0",(float)(n6*6)*(float)(xtra),
        ...
        pout("N7 fp assignments   \0",(float)(n7*3)*(float)(xtra),
                            1,e1[2],timeb,calibrate,7);
        ...
        pout("N8 exp,sqrt,log,div. \0",(float)(n8*4)*(float)(xtra),
                                1,x,timeb,calibrate,8);
    ...
    void pout(char title[18], float ops, int type, SPDP checknum,
              SPDP time, int calibrate, int section)
      {
        ...
        if (calibrate == 0)
          {              
            printf("%-22s %20.17f    ",headings[section],results[section]);    
